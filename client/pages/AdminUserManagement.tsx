...
import { ArrowLeft, UserPlus, Users, AlertCircle, Edit, Trash2, Shield } from 'lucide-react';
...
export default function AdminUserManagement() {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(`✅ Utilisateur "${formData.email}" créé avec succès.`);
      setFormData({
        email: '',
        password: '',
        nom: '',
    ...
    try {
      ...
      if (formData.role === 'admin' && formData.fermeId && formData.fermeId !== 'all') {
        try {
          ...
          setMessage(`Utilisateur mis à jour: ${editingUser.email} (Erreur lors de l'ajout aux admins)`);
        }
      } else {
        setMessage(`Utilisateur mis à jour: ${editingUser.email}`);
      }
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="text-sm font-semibold text-gray-900">{allUsers?.length || 0}</span>
              </div>
              <div className="flex justify-between">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            ...
            Liste des utilisateurs ({allUsers?.length || 0})
          </CardTitle>
        </CardHeader>
        <CardContent>
